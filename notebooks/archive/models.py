...
class GraphLearning(nn.Module):
    ...
    def forward(self):
            ...
            sim = self.v @ self.W @ self.v.T
            
            # 2. Apply Sigmoid (Range 0 to 1)
            A = torch.sigmoid(sim)
            
            # 3. (Optional) Apply your Threshold/Masking here
            # mask = (A > 0.3).float()
            # A = A * mask
            A = F.leaky_relu(A)

            # 4. Force Diagonal to 1.0
            # Create an Identity matrix (1s on diagonal, 0s elsewhere)
            eye = torch.eye(A.size(0)).to(A.device)
            
            # Logic: (Keep non-diagonal values) + (Add 1s to diagonal)
            # (A * (1 - eye)) -> Sets diagonal of A to 0
            # + eye           -> Adds 1 to the diagonal
            A = A * (1 - eye) + eye
...
class TR_GNN_MultiScale(nn.Module):
    def __init__(self, N, T_in, T_out, d=32, hidden_dim=64, 
        ...
        # Use the IMPROVED TemporalConv from the previous step
        self.temporal_conv = TemporalConv(N, T_in, hidden_dim, 
                                          dilation=3, dropout=dropout_temporal)
        
        # Use the IMPROVED DenselyResidualGCN from the previous step
        self.dense_gcn = DenselyResidualGCN(hidden_dim, hidden_dim, 
                                            dropout=dropout_gcn, layers=GCN_Layer)
        
        # NEW: Updated Forecaster
        self.forecaster = MultiScaleForecasting(
            N=N,
            T_in=T_in,
            T_out=T_out,
    ...
    def forward(self, X):  # X: (B, N, T_in)
        ...
        # 2. Temporal Processing (Extract Features)
        # Output H is (B, N, hidden_dim) - representing the "State" at the last timestep
        H = self.temporal_conv(X)
        
        # 3. Spatial Processing (Mix Features via Graph)
        H = self.dense_gcn(H, A)
        
        # 4. Forecast (Combine Raw Trend + Deep Residuals)
        # We pass 'X' (original input) so the model knows the recent history values
